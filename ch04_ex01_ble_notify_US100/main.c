...
TaskHandle_t US100TaskHandle = NULL;
	...
	while(1){
		...
		if (connection_id != 0)
		{
			...
			if( app_psoc_us100_client_char_config[0] & GATT_CLIENT_CONFIG_NOTIFICATION )
			{
					...
					sprintf(buffer_text,"Value US100 = %d",Distance);
	...
	while(true)
	{
		...
		if (ulNotificationValue == 1)
		{
			if( connection_id ) /* Check if we have an active connection */
			{
				...
				if( app_psoc_button_count_2_client_char_config[0] &
				GATT_CLIENT_CONFIG_NOTIFICATION )
				{
					printf( "Notify button press count: (%d)\n",
					app_psoc_button_count[0] );
...
int main(void)
{
    ...
    xTaskCreate (us100_task, "us100Task", TASK_STACK_SIZE, NULL, TASK_PRIORITY-1, &US100TaskHandle);
