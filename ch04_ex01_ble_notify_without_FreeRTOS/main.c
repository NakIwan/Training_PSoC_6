...
volatile bool pressFlagIn = false;
volatile bool pressFlagDe = false;
...
int main(void)
{
    ...
    cyhal_gpio_register_callback(P0_4,&app_button_isr_data);
    cyhal_gpio_enable_event(P0_4, CYHAL_GPIO_IRQ_FALL, 7, true);
    		...
    		else if( app_psoc_button_count_2_client_char_config[0] & GATT_CLIENT_CONFIG_NOTIFICATION )
    		{
    			if (pressFlagDe){
        			...
        			wiced_bt_gatt_server_send_notification( connection_id,
        													...
        													app_psoc_button_count_2_len,
        													app_psoc_button_count,
        													NULL);
    			}
    		}
    		else if(app_psoc_us100_client_char_config[0] & GATT_CLIENT_CONFIG_NOTIFICATION){
				if (cyhal_uart_putc(&uart_obj, 0X55) == CY_RSLT_SUCCESS){
							rx_length = 3;
							cyhal_uart_read(&uart_obj, rx_buf, &rx_length);
							uint16_t data = (uint16)rx_buf[0] << 8 |rx_buf[1];
							sprintf(buffer_text,"US100 = %d\r\n",data);
							wiced_bt_gatt_server_send_notification( connection_id,
...
static wiced_bt_gatt_status_t app_bt_server_event_handler(wiced_bt_gatt_event_data_t *p_data)
{
    ...
    switch (p_att_req->opcode)
    {
		...
		case GATT_CMD_SIGNED_WRITE:
			...
			printf("Value Id = %d\n\r",write_param.handle);
			printf("p_val0 %d\r\n",write_param.p_val[0]);

			if(write_param.handle == HDLD_PSOC_BUTTON_COUNT_CLIENT_CHAR_CONFIG)
				button_notify = write_param.p_val[0];
			if(write_param.handle == HDLD_PSOC_BUTTON_COUNT_2_CLIENT_CHAR_CONFIG)
				button_notify = write_param.p_val[0];
			if(write_param.handle == HDLD_PSOC_US100_CLIENT_CHAR_CONFIG)
				button_notify = write_param.p_val[0];

			printf("Button Notify = %d\r\n",button_notify);
